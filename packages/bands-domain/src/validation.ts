...
import {
  BAND_INTERNAL_KIT_LINK_KINDS,
  BAND_PUBLIC_SECTION_KEYS,
  normalizeBandSectionOrder,
} from './presentation'
...
const timelineIconSchema = z.string().trim().min(1)
...
const publicSectionKeySchema = z.enum(BAND_PUBLIC_SECTION_KEYS)

const presentationSchema = z.object({
  sectionOrder: z
    .array(publicSectionKeySchema)
    .default(normalizeBandSectionOrder())
    .transform((value) => normalizeBandSectionOrder(value)),
})

const internalKitLinkSchema = z.object({
  _key: itemKeySchema,
  label: requiredTrimmedString,
  url: requiredUrlSchema,
  kind: z.enum(BAND_INTERNAL_KIT_LINK_KINDS).default('other'),
})
...
const internalKitSchema = z.object({
  ...
  bookingNotes: optionalTrimmedString,
  keyLinks: z.array(internalKitLinkSchema).default([]),
})
...
export const bandUpdateSchema = z.object({
  ...
  presentation: presentationSchema.default({
    sectionOrder: normalizeBandSectionOrder(),
  }),
...
export function toSanityBandPatch(input: BandUpdateInput, syncedAt = new Date().toISOString()) {
  return {
    ...
    gallerySection: hasGalleryContent(input.gallerySection)
      ...
      : undefined,
    presentation: {
      sectionOrder: normalizeBandSectionOrder(input.presentation.sectionOrder),
    },
    internalKit: hasInternalKitContent(input.internalKit)
      ? compactObject({
          ...
          bookingNotes: input.internalKit.bookingNotes,
          keyLinks: input.internalKit.keyLinks.map((link) =>
            compactObject({
              _key: link._key,
              _type: 'object',
              label: link.label,
...
export function toSanityBandSet(input: BandUpdateInput, syncedAt?: string) {
  ...
  const values: Record<string, unknown> = {
    ...
    'internalKit.bookingNotes': patch.internalKit?.bookingNotes,
    'internalKit.keyLinks': patch.internalKit?.keyLinks,
...
export function toSanityBandUnset(input: BandUpdateInput) {
  const optionalValues: Record<string, unknown> = {
    ...
    'internalKit.bookingNotes': input.internalKit.bookingNotes,
  ...
  if (!hasInternalKitContent(input.internalKit)) {
    unsetPaths.push('internalKit')
  }
